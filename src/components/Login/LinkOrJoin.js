...
function Anonymous({ providerId, label, email, displayName, join }) {
  ...
  const leadMsg = `You've successfully authenticated by using your ${label}
    ${providerId} account. Unfortunately we did not find an exsisting user
  ...
  const linkMsg = `Enter the email address of an exsisting user you want to
    link this login to.`
  ...
  return (
    <div>
      ...
      { email &&
        <button onClick={() => join({ displayName, email })}>
