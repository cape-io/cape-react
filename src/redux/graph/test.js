...
import { getSXX } from './select'
import reducer, { put } from './index'
...
test('reducer and put action', assert => {
  ...
  const result = reducer(undefined, put(timEmailArray))
  assert.deepEquals(result, expected, 'result matches expected')
  const result2 = reducer(undefined, put(timEmailObject))
